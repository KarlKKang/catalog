...
if [[ "$1" == "featherine-website" ]]; then
    ...
    node ./aws-s3-js/upload.js "$1" "/" "$2/" --dry-run $opt $REGION_OPT 2>>"publish.log" || exit 1
    echo
    node ./aws-s3-js/upload.js "$1" "/" "$2/" \
        ...
        $opt $REGION_OPT 2>>"publish.log" || exit 1
    echo "Waiting 10 seconds for the CloudFront error TTL to expire..."
    sleep 10
    echo
    node ./aws-s3-js/upload.js "$1" "/index.html" "$2/index.html" --mime '\.html$' 'text/html' $REGION_OPT 2>>"publish.log" || exit 1
    node ./aws-s3-js/upload.js "$1" "/version" "$2/version" --mime '.*' 'text/plain;charset=utf-8' $REGION_OPT 2>>"publish.log" || exit 1
    ...
    echo
    node ./aws-s3-js/upload.js "$1" "/sw.js" "$2/sw.js" --mime '\.js$' 'application/javascript;charset=utf-8' $REGION_OPT 2>>"publish.log" || exit 1
    ...
    node ./aws-s3-js/upload.js "$1" "/" "$2/" \
        ...
        --delete $REGION_OPT 2>>"publish.log"
fi
